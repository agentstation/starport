...
// NewKeyManager creates a new provider key manager
func NewKeyManager(store storage.KVStore, masterKey []byte) (KeyManager, error) {
	if store == nil {
...
func (m *keyManager) AddKey(ctx context.Context, scope, provider string, key map[string]string, config map[string]interface{}, isFallback bool, priority int) (*models.ProviderKey, error) {
	// Validate inputs
	if scope == "" {
...
func (m *keyManager) GetKeys(ctx context.Context, scope, provider string) ([]*models.ProviderKey, error) {
	...
	userPrefix := fmt.Sprintf("credential:%s:", scope)
...
func (m *keyManager) ListKeys(ctx context.Context, scope string) ([]*models.ProviderKey, error) {
	...
	prefix := fmt.Sprintf("credential:%s:", scope)
...
func (m *keyManager) UpdateKey(ctx context.Context, scope, provider string, key map[string]string, config map[string]interface{}, isFallback *bool, priority *int) (*models.ProviderKey, error) {
	// Get existing key
	storageKey := models.ProviderKeyStorageKey(scope, provider)
	data, err := m.store.Get(ctx, storageKey)
	if err != nil {
...
func (m *keyManager) AddGlobalKey(ctx context.Context, provider string, key map[string]string, config map[string]interface{}, rateLimit *models.RateLimitConfig) (*models.ProviderKey, error) {
	if provider == "" {
		return nil, ErrProviderRequired
	}
...
func (m *keyManager) UpdateGlobalKey(ctx context.Context, provider string, key map[string]string, config map[string]interface{}, rateLimit *models.RateLimitConfig) (*models.ProviderKey, error) {
	// Get existing key
	storageKey := models.GlobalProviderKeyStorageKey(provider)
	data, err := m.store.Get(ctx, storageKey)
	if err != nil {
