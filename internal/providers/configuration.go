...
// Configuration binds optional operator settings and one request-time
// material source to an exact catalog provider. It contains no credential
// value and does not determine provider membership.
...
func Availability(activations []Activation) []runtimecatalog.AdapterAvailability {
	...
	for _, activation := range activations {
		result = append(result, runtimecatalog.AdapterAvailability{
			ProviderID: activation.ProviderID, Registered: true,
