...
func TestSharedCacheRealService(t *testing.T) {
	...
	// A read must not create a local copy when the backing entry has no expiry.
	require.NoError(t, control.Do(ctx, control.B().Set().Key(first.prefix+"unknown-lifetime").Value("temporary").Build()).Error())
	value, found, err = second.Get(ctx, "unknown-lifetime")
	require.NoError(t, err)
	require.True(t, found)
	require.Equal(t, "temporary", string(value))
	require.NoError(t, control.Do(ctx, control.B().Del().Key(first.prefix+"unknown-lifetime").Build()).Error())
	_, found, err = second.Get(ctx, "unknown-lifetime")
	require.NoError(t, err)
	require.False(t, found, "shared reads must not retain unknown-lifetime entries locally")
