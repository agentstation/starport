...
// it reads back when one arrives. Both spellings below are the OpenAI ones, so
// an SDK written against that API works without a translation layer.

// TestDecodeOutputModalityRequest holds the request spelling.
func TestDecodeOutputModalityRequest(t *testing.T) {
	body := `{"model":"gpt-4o-audio-preview",` +
		`"modalities":["text","audio"],` +
		`"audio":{"voice":"alloy","format":"wav"},` +
		`"messages":[{"role":"user","content":"say hello"}]}`

	request, err := DecodeChat(strings.NewReader(body))
