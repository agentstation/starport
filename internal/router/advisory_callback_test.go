...
func TestAdvisoryLatencyCallbacksAvoidStorage(t *testing.T) {
	for _, name := range []string{"cold_read", "all_reads", "record"} {
		t.Run(name, func(t *testing.T) {
			...
			assertAdvisoryCallbackLocal(t, store, func() {
				switch name {
				case "cold_read":
					tracker.GetLatency("provider")
				case "all_reads":
					tracker.GetAllLatencies()
				case "record":
					tracker.RecordLatency("provider", time.Millisecond)
				}
			})
		})
	}
