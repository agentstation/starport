...
func TestEncryptionService_LongMasterSecretRoundTrip(t *testing.T) {
	service, err := NewEncryptionService([]byte(strings.Repeat("master-secret", 6)))
	if err != nil {
		t.Fatalf("NewEncryptionService() error = %v", err)
	}
	encrypted, err := service.EncryptCredential("provider-secret")
	if err != nil {
	...
	if decrypted != "provider-secret" {
		t.Fatalf("DecryptCredential() = %q, want provider-secret", decrypted)
	}
}
