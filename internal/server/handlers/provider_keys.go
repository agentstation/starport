...
func (h *ProviderKeysHandler) Create(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	apiKeyID := chi.URLParam(r, "key_id")
	...
	if validateParam := r.URL.Query().Get("validate"); validateParam == "true" {
		if err := h.manager.ValidateKey(ctx, req.Provider, credMap, req.Config); err != nil {
	...
	// Store the key
	_, err := h.manager.AddKey(ctx, "user:"+apiKeyID, req.Provider, credMap, req.Config, req.IsFallback, req.Priority)
	if err != nil {
...
func (h *ProviderKeysHandler) Get(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	apiKeyID := chi.URLParam(r, "key_id")
...
func (h *ProviderKeysHandler) Update(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	apiKeyID := chi.URLParam(r, "key_id")
	provider := chi.URLParam(r, "provider")
	...
	// Check if key exists
	_, err := h.manager.GetKey(ctx, "user:"+apiKeyID, provider)
	if err != nil {
		if err == storage.ErrNotFound {
			dto.WriteError(w, http.StatusNotFound, dto.ErrorTypeNotFound, "Provider key not found")
			return
	...
	if credMap != nil && r.URL.Query().Get("validate") == "true" {
		if err := h.manager.ValidateKey(ctx, provider, credMap, req.Config); err != nil {
	...
	// Update the key
	_, err = h.manager.UpdateKey(ctx, "user:"+apiKeyID, provider, credMap, req.Config, req.IsFallback, req.Priority)
	if err != nil {
...
func (h *ProviderKeysHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.manager.DeleteKey(ctx, "user:"+apiKeyID, provider); err != nil {
		if err == storage.ErrNotFound {
			dto.WriteError(w, http.StatusNotFound, dto.ErrorTypeNotFound, "Provider key not found")
			return
		}
...
func (h *ProviderKeysHandler) Validate(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	apiKeyID := chi.URLParam(r, "key_id")
	provider := chi.URLParam(r, "provider")
...
func (h *ProviderKeysHandler) GetUsage(w http.ResponseWriter, _ *http.Request) {
