  ...
  return (
    <span className={cn("relative inline-flex", className)}>
      <select
        {...props}
        className={cn(
          "w-full appearance-none rounded-sm border border-border-2 bg-bg-raised text-text-1 outline-none transition-colors duration-150 ease-standard hover:border-border-3 focus-visible:border-accent disabled:cursor-not-allowed disabled:opacity-60",
          sizing,
        )}
      >
        {children}
      ...
      <ChevronDown
        aria-hidden="true"
        className={cn(
          "pointer-events-none absolute top-1/2 -translate-y-1/2 text-text-4",
